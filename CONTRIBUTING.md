...
Some maintainers run LV3 from an Android phone using **Termux** + Codex CLI when away from a laptop.
This is optional; contributors do not need Termux.

- Termux is a Linux-like environment on Android where you can run `git`, `python`, and the LV3 scripts.
- Heavy ML dependencies (Torch/FAISS/SONAR) may be harder on Android than on desktop; if installs fail, use a laptop/desktop for embedding runs and keep Termux for docs/light tasks.
