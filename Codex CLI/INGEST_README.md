...
## New utility scripts
- `scripts/convert_stardict.py` — convert extracted Wiktionary StarDict bundles to JSONL (`lemma`, `gloss`, `language`, `source=wiktionary-stardict`, `lemma_status=auto_brut`). Example:  
  `python scripts/convert_stardict.py --root data/raw/wiktionary_extracted --out data/processed/wiktionary_stardict`
- `scripts/ingest_quran_morphology.py` — parse Quranic Arabic Corpus morphology mirror into unique lemma/root list. Example:  
  `python scripts/ingest_quran_morphology.py --input data/raw/arabic/quran-morphology/quran-morphology.txt --output data/processed/arabic/quran_lemmas.jsonl`
